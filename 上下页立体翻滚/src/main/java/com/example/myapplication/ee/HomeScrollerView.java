package com.example.myapplication.ee;
...
public class HomeScrollerView extends ViewGroup {
    ...
    public HomeScrollerView(Context context) {
        this(context, null);
    }

    public HomeScrollerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HomeScrollerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mDownX = x;
                mTempY = mDownY = y;
                if (!mScroller.isFinished() && isMyScroll) {
...
//                    mState = State.ToPre;
    ...
    private HomeScrollerView.State mNewState = State.ToNext;
...
//        Log.e("ImageView", "startY = " + startY+"       delta = " + delta+"        duration = " + duration);
        mScroller.startScroll(0, startY, 0, delta, duration);
//        Log.e("ImageView", "startY = " + startY+"       delta = " + delta+"        duration = " + duration+"        startY = " + startY);
        addCount--;
    }
...
//        addCount = flingSpeedCount / flingSpeed + 1;
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            isMyScroll = false;
//            Log.e("ImageView", "滑动中" + "  isMyScroll " + isMyScroll);
        ...
        if (mScroller.isFinished()) {
            ...
            addCount = 0;
            isMyScroll = true;
//            Log.e("ImageView", "结束" + "  isMyScroll " + isMyScroll);
        }
    }

    boolean isMyScroll = true;
    ...
    private void recycleMove(int delta) {
        ...
        if (mNewState == State.ToNext && delta > 0 || mNewState == State.ToPre && delta < 0) {
            return;
        }
        scrollBy(0, delta);
...
//        }
     ...
     * @return
     */
    public HomeScrollerView setStartScreen(int startScreen) {
     ...
     * @return
     */
    public HomeScrollerView setResistance(float resistance) {
     ...
     * @return
     */
    public HomeScrollerView setInterpolator(Interpolator mInterpolator) {
     ...
     * @return
     */
    public HomeScrollerView setAngle(float mAngle) {
     ...
     * @return
     */
    public HomeScrollerView setCan3D(boolean can3D) {
        isCan3D = can3D;
        return this;
    }
     ...
     * @return
     */
    public HomeScrollerView setItem(int itemId) {

        if (!mScroller.isFinished()) {
            mScroller.abortAnimation();//强制完成
        }
     ...
     * @return
     */
    public HomeScrollerView toNext() {
        if (!mScroller.isFinished()) {
            mScroller.abortAnimation();//强制完成
        }
