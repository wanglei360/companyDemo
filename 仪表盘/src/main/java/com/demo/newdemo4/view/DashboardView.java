...
public class DashboardView extends View {
     ...
     * 值域（mMax-mMin）等分份数
     */
    private int mSection = 18;
    /**
     * 一个mSection等分份数
     */
    private int mPortion = 18;
    /**
     * 表头
     */
    ...
    public float mCenterX, mCenterY; // 圆心坐标
    ...
    private void init() {
        ...
        //初始化一个画笔
        rootRadiusPaint = new Paint();
        rootRadiusPaint.setStrokeWidth(5);//画笔的宽度
        rootRadiusPaint.setColor(ContextCompat.getColor(getContext(), R.color.dashboard_background));
        rootRadiusPaint.setAntiAlias(true);// 设置画笔的锯齿效果。 true是去除，大家一看效果就明白了
...
//        Log.d("onDraw", "θ = " + θ);
        ...
        //画指针根部的半圆
        RectF rect = new RectF(mCenterX-100, mCenterY-100, mCenterX+100, mCenterY+130);
        ...
        if (isShowValue) {
            ...
            mPaint.setTextAlign(Paint.Align.CENTER);
            mPaint.setColor(ContextCompat.getColor(getContext(), R.color.textColor));
            String value = String.valueOf(mRealTimeValue)+"kg";
            mPaint.getTextBounds(value, 0, value.length(), mRectText);
            canvas.drawText(value, mCenterX, mCenterY-30, mPaint);
        }
    }
